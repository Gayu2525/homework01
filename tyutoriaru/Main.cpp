...
void Main()
{
	...
	while (System::Update())
	{
		if (gamestart == false && MouseL.down())
		{
			gamestart = true;
		}

		if (gamestart == false) {
			font(U"左クリック"_fmt(count)).draw(80, Vec2{ 200, 200 }, ColorF{ 0.2, 0.6, 0.9 });
		}

		if (gamestart == true) {
			// パドル | Paddle
			const Rect paddle{ Arg::center(Cursor::Pos().x, 500), paddlesize, 10 };
